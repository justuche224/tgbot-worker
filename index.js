...
3️⃣ Allow up to 24 hours for review.
    ...
    extra: Markup.inlineKeyboard([
      [
        Markup.button.url(
          "📄 KYC Docs",
          "https://ecohavest.org/dashboard/account/kyc"
        ),
      ],
...
export async function fetchCryptoNews() {
  if (!NEWS_API_KEY) {
    console.error(
      "NewsAPI key is missing. Please set the NEWS_API_KEY environment variable."
    );
    return null;
  }
  try {
    ...
    if (response.data.status === "ok") {
      ...
      console.error(
        `NewsAPI returned status: ${response.data.status}`,
...
const allCommands = [
  ...
  { command: "getid", description: "Get this chat’s ID" },
...
bot.command("crypto_updates", async (ctx) => {
  ...
  try {
    ...
    if (newsData) {
      ...
      newsMessage =
        "Could not retrieve cryptocurrency news (check API key and network).";
      console.error(
        "Failed to get news data for combined message (Network/Key Error)."
      );
    }
    // Combine messages
    const combinedMessage = `${cryptoMessage}\n\n${newsMessage}`;
...
bot.command("getid", (ctx) => {
  // ctx.chat.id will be the numeric identifier for the chat,
  // whether it’s a private chat, group or channel.
  ctx.replyWithHTML(`<b>This chat’s ID is:</b> <code>${ctx.chat.id}</code>`);
});
